...
- Updating issue state (status, description, labels) is allowed only when explicitly instructed — **except** for the workflow status transitions defined in the Linear Workflow Status Management section below, which happen automatically.
...
|---|---|---|
| Picks up the issue | → **In Progress** | — |
| Work complete, needs review | → **In Review** | Subscribe the user (so it appears in their Inbox) |
| Blocked on external dependency or real-world action only the user can take | → **Blocked** | Assign to user; set priority Urgent |
