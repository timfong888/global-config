...
If the invoking repo's `CLAUDE.md` has no `## Agent Poll Configuration` block **and** you're not deliberately using the Satchel default below, stop and report a configuration error rather than guessing an id — a wrong state/team id silently mis-routes or silently no-ops every call downstream.
