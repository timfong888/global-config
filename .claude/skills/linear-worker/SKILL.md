...
**Progress file — initial phase plan (local only, no Linear calls):**
```shell
python3 tests/lib/progress_file.py write --dir state --issue <identifier> \
  --phases-total <N> --now "<what you're about to do first>"
```
`phases_total` = your honest count of natural checkpoints in the B3 profile. Best-effort only;
...
**Progress file — phase boundaries:**
```shell
python3 tests/lib/progress_file.py write --dir state --issue <identifier> \
  --phases-done <n> --completed "<what's finished>" \
  --now "<what's in flight>" --estimate "<phase n/total, roughly ...>"
```
Fields you don't pass keep their prior value (merge semantics). Best-effort; never a gate.
