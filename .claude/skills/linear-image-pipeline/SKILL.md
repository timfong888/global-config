...
**Model choice within `fal_ai`:** prefer open-weight, commercially-unrestricted models over Gemini.
...
A native `Read`-style review tool needs a local path, not a remote URL. Download the hosted URL to `generated-images/<issue-id>/<descriptive-name>.<ext>` (create the directory if missing, gitignore it — never commit these), verify the file is non-empty, then hand that **local path** to the review step. Skip this stage only when the image is never reviewed before attaching.
...
- **Prefer Linear's own native file-upload path** (`prepare_attachment_upload` + `create_attachment_from_upload`, or equivalent — i.e. an `uploads.linear.app` asset) when the session's Linear access exposes it. It's simpler (no separate storage account) and is the path confirmed reliable for images that need to **render inline** in a Linear comment/description — an externally-hosted link (Drive, catbox, tmpfiles, …) has repeatedly failed to render inline even when reachable directly.
- If native upload isn't available, fall back to uploading to a connected durable-storage account (e.g. Google Drive via an upload-from-URL action) and confirm the resulting share permission is anyone-with-the-link view — otherwise the Linear attachment preview hits a permission wall. **This fallback is unverified for inline rendering** — treat it as attachment-only, not a substitute for native upload where inline display matters.
