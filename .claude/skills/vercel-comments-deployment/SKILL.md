...
git commit --only -- <changed files> -m "Apply Vercel toolbar feedback: $SUMMARY"
...
1. `list_deployments` for the project, `since` = the merge timestamp, to get candidate IDs.
...
Only after step 9's Issue URL is in hand. Per actioned thread: `reply_to_toolbar_thread` with "Change applied — [description]. Deployed in [PR link]. Summary: [Issue URL]." then `change_toolbar_thread_resolve_status` to resolved.
